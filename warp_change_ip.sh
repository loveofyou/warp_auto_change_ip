...
if [[ "$input" == "n" ]];then
    bash <(curl -6 -fsSL https://github.com/luoxue-bot/warp.sh/raw/main/warp.sh) 4
elif [[ "$input" == "y" ]];then
...
do
    ...
    if [[ "$result" == "404" ]];then
		...
		region=`tr [:lower:] [:upper:] <<< $(curl -6 --user-agent "${UA_Browser}" -fs --max-time 10 --write-out %{redirect_url} --output /dev/null "https://www.netflix.com/title/80018499" | cut -d '/' -f4 | cut -d '-' -f1)` ;
		if [[ ! "$region" ]];then
			region="US";
		fi
        if [[ "$region" != "$area" ]];then
